...
function TexturedCabinet({ config, showDimensions }) {
...
function CabinetModel({ config, showDimensions }) {
  ...
  return (
    <Suspense fallback={null}>
      <TexturedCabinet config={config} showDimensions={showDimensions} />
    </Suspense>
  );
}
...
function CabinetConfigurator() {
  ...
  const openARViewer = () => {
    // In a real implementation, you might want to generate a specific model path
    // based on the current configuration, or use a server endpoint that can
    // dynamically generate the model
        ...
        <div className="flex-1 flex flex-col md:flex-row">
          <div className="w-full md:w-3/5 h-[50vh] md:h-auto flex items-center justify-center bg-gray-50">
            <Canvas shadows dpr={[1, 2]} gl={{ preserveDrawingBuffer: true }}>
              ...
              <Suspense fallback={null}>
                <CabinetModel config={config} showDimensions={showDimensions} />
