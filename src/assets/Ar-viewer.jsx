...
const ARViewer = ({ visible, onClose, modelPath, productConfig }) => {
  ...
  useEffect(() => {
    if (visible) {
      ...
      // Add all configuration parameters from productConfig
      if (productConfig) {
        console.log("Sending product config to AR view:", productConfig);
        Object.entries(productConfig).forEach(([key, value]) => {
      ...
      // Set the complete AR view URL
      const fullUrl = `${baseUrl}/ar-view.html?${params.toString()}`;
      console.log("Generated AR view URL:", fullUrl);
      setArViewUrl(fullUrl);
    }
  }, [visible, modelPath, productConfig]);
